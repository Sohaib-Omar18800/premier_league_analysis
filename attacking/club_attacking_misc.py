...
with sync_playwright() as p:
    ...
    new_page.goto(url=url, wait_until="commit", timeout=100000)
    new_page.locator("text=teams").click(timeout=100000)
    new_page.locator("text=misc").click(timeout=100000)
    new_page.wait_for_selector("table")
    heads = new_page.locator("table th").all_inner_texts()
    with open(r"D:/Next Academy/My Own Project/football/attacking/club_attacking_misc_25-26.csv", mode="w+") as f:
        f.write("club_name,total_matches,tou_in_box,hit_post,offsides,total_penalties,penalties_goals,toal_freekicks,freekicks_goals,total_headers,headers_goals,total_fast_breakes,fast_breaks_goals,club_id\n")
        rows = new_page.locator("table tbody tr").all()
        for row in rows:
