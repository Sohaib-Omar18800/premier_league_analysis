...
with sync_playwright() as p:
    ...
    page.locator("text=DISCIPLINE").click(timeout=100000)
    page.locator("text=filter players").click(timeout=100000)
    ...
    with open(r"D:/Next Academy/My Own Project/football/player_stats/players_discipline_25-26.csv", "w+", encoding='utf-8') as f:
        for header in headers:
            if header != last_index:
                f.write(f"{header.replace(' ','_').lower()},")
            else:
                f.write(
                    f"{header.replace(' ','_').lower()},player_id,club_id,club_name\n")
