...
with sync_playwright() as p:
    ...
    page.get_by_role("option", name="Sequences").click(timeout=100000)
    page.wait_for_selector('table')
    rows = page.locator("table tbody tr").all()
    with open(r"D:/Next Academy/My Own Project/football/sequences/club_sequence_25-26.csv", mode="w+") as f:
        f.write('club_name,matches_played,over_ten_passes,direct_speed,passes_per_seq,sequence_time,build_ups_total,build_ups_goals,direct_attack_total,direct_attack_goal,club_id\n')
        for row in rows:
