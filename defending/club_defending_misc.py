...
with sync_playwright() as p:
    ...
    page.locator("text=MISC").click(timeout=100000)
    page.wait_for_selector('table')
    rows = page.locator("table tbody tr").all()
    with open(r"D:/Next Academy/My Own Project/football/defending/club_defending_misc_overall_25-26.csv", mode="w+") as f:
        c = 1
        f.write('club_name,matches_played,tou_in_box,hit_post,offsides,total_penalties,penalties_goals,total_freekicks,freekicks_goals,total_headers,headers_goal,total_fast_breaks,fast_breaks_goals,club_id\n')
        for row in rows:
