...
with sync_playwright() as p:
    ...
    page.locator("text=DEFENSIVE ACTIONS").click(timeout=100000)
    page.wait_for_selector('table')
    headers = page.locator("table th").all_inner_texts()
    rows = page.locator("table tbody tr").all()
    with open(r"D:/Next Academy/My Own Project/football/defending/club_defensive_actions_25-26.csv", mode="w+") as f:
        c = 1
        for header in headers:
            if header == headers[-1]:
                f.write(f"{header.replace(' ','_').lower()},club_id\n")
            else:
