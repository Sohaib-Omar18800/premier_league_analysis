...
with sync_playwright() as p:
    ...
    page.get_by_role("option", name="MISC.").click(timeout=100000)
    page.wait_for_selector('table')
    headers = page.locator("table th").all_inner_texts()
    headers[0] = 'club_name'
    rows = page.locator("table tbody tr").all()
    with open(r"D:/Next Academy/My Own Project/football/misc/club_misc_25-26.csv", mode="w+") as f:
        c = 1
        for header in headers:
            if header == headers[-1]:
                f.write(f"{header.lower().replace(' ','_')},club_id\n")
            else:
                f.write(f"{header.lower().replace(' ','_')},")
        for row in rows:
