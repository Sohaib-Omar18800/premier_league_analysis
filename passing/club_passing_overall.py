...
with sync_playwright() as p:
    ...
    with open(r"D:/Next Academy/My Own Project/football/passing/club_sequence_passing_overall_25-26.csv", mode="w+") as f:
        f.write("club_name,matches_played,avg_poss,total_all_passes,successful_all_passes,all_passes_percentage,total_final_third_passes,successful_final_third_passes,final_third_percentage,fwd_pass_direction,bwd_pass_direction,left_pass_direction,right_pass_direction,total_crosses,successful_crosses,crosses_percentage,through_balls,club_id\n")

        for row in rows:
            row_list = row.locator("td").all_inner_texts()
            counter = 1
            club_id = club_dic.get(row_list[0])
            club_name = row_list[0]
